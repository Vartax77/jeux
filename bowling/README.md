...
- **« Calibrer le lift (2 lancers) »** : *lance en LIFTANT VERS LA GAUCHE*, puis *vers la DROITE*, à fond. Le jeu en déduit le **sens** (quel sens de rotation du poignet envoie la boule à gauche, selon ta main et ta prise — plus de réglage « sens de l'effet » à deviner), le **plein effet** (90 % du côté le plus faible : ton lift maximal donne un crochet plein des deux côtés) et une zone morte de 15 % (un poignet à peu près droit donne une boule droite). Si les deux lancers tournent dans le même sens, ou trop peu, le calibrage est refusé et le profil ne bouge pas.
...
### Sons
Tout est synthétisé. Pour remplacer un son, dépose un fichier `sons/<nom>.ogg` (ou `.mp3`, `.wav`), nom exact parmi : `roulement`, `impact-faible`, `impact-moyen`, `impact-fort`, `gouttiere`, `rebond`, `pinsetter`, `foule-acclamation`, `foule-ovation`, `foule-oh`, `foule-rire`, `clic`, `jingle-strike`, `jingle-spare`, `jingle-turkey`, `jingle-parfait`, `musique` (boucle), et les chocs joués sur les contacts réels : `choc-boule-quille`, `choc-quille-quille`, `choc-quille-sol`, `choc-quille-paroi`, `choc-boule-paroi`, `choc-boule-sol` (courts, 50–150 ms ; le jeu module leur volume et leur hauteur selon la vitesse d'impact). Sources CC0 conseillées : freesound.org (filtre « Creative Commons 0 »). Volumes et musique dans Réglages → Audio.
